...
<label>最低经历支持度（0—100）<input id="policy-score" type="number" min="0" max="100" value="{policy['min_score']}"></label></div>
...
def render_report(r):
    ...
    for row in r["jobs"]:
        ...
        rows.append(f'<tr><td><a href="{e(job["url"])}" target="_blank" rel="noopener noreferrer">{e(job["title"])}</a><small>{e(job["company"])} · {e(job.get("city") or "城市未知")}</small>{preview}</td><td>{a["score"]}%<small>{e(source)}</small></td><td>{expr}</td><td>{labels[a["status"]]}<small>{e(reasons)}</small></td></tr><tr class="detail"><td colspan="4"><b>匹配证据：</b>{e(evidence)}<br><b>缺口：</b>{e(missing)}</td></tr>')
    cards = []
    for s in r["suggestions"]:
        ...
        cards.append(f'<article data-id="{s["id"]}"><div class="cardtop"><span class="tag">{kind}</span><small>{ai} · {e(s["fact_id"] or "无对应经历")}</small></div><h3>{e(job["title"])} · {e(job["company"])}</h3><p>{e(s["reason"])}</p>{quote}<p>{e(s["question"])}</p>{editor}<label>本条处理方式 <select id="action-{s["id"]}">{options}</select></label></article>')
    search = "".join(f'<li><a href="{e(q["url"])}" target="_blank" rel="noopener noreferrer">{e(q["query"])}</a><small>{e(q.get("city") or "城市不限")} · 依据 {e(", ".join(q["evidence_ids"]))}</small></li>' for q in r["search_plan"]["queries"])
...
document.getElementById('download-policy').addEventListener('click', () => {
  try {
    ...
    if (!Number.isFinite(p.min_monthly_salary) || p.min_monthly_salary < 0 || !Number.isFinite(p.min_score) || p.min_score < 0 || p.min_score > 100 || (p.max_monthly_salary !== null && (!Number.isFinite(p.max_monthly_salary) || p.max_monthly_salary < p.min_monthly_salary))) throw new Error('请检查薪酬区间和分数。');
...
<div class="summary"><div><small>已分析岗位</small><b>{r['summary']['total']}</b></div><div><small>建议关注</small><b>{r['summary']['matched']}</b></div><div><small>待核实</small><b>{r['summary']['needs_review']}</b></div><div><small>已排除</small><b>{r['summary']['rejected']}</b></div></div>
