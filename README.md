...
**当前版本：0.1.0。核心流程可运行；BOSS / 猎聘真实页面投递尚未验收。**
首版是 Python 命令行工具 + Edge/Chrome 岗位采集扩展，没有独立网页后台。
...
| 城市、月薪下限、职位、排除公司过滤 | 已实现；硬条件未知进入待核实 |
...
岗位年限、学历要求等尚未自动结构化核验，应通过人工审阅后再执行申请。
...
此版本不会直接根据 JD 自由生成新履历；需要更换表述时先补充已确认改写。
